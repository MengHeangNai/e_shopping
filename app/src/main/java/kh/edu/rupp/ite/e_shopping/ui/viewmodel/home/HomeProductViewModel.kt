...
private const val TAG = "HomeProductViewModel"
...
class HomeProductViewModel(
    ...
    val mostRequestedClothes = MutableLiveData<Resource<List<Product>>>()
    ...
    fun getMostRequestedChairs(size: Int = 0) {
        if (mostRequestedChairsProducts != null && size == 0) {
            mostRequestedClothes.postValue(Resource.Success(chairsProducts))
            return
        }
        mostRequestedClothes.postValue(Resource.Loading())
        shouldPaging(CUPBOARD_CATEGORY, size) { shouldPaging ->
            if (shouldPaging) {
                chairs.postValue(Resource.Loading())
                    ...
                    .addOnCompleteListener {
                        if (it.isSuccessful) {
                            ...
                            if (!documents!!.isEmpty) {
                                ...
                                mostRequestedClothes.postValue(Resource.Success(productsList))
                                mostRequestedChairsPage += 4

                            }
                        } else
                            mostRequestedClothes.postValue(Resource.Error(it.exception.toString()))
                    }
            } else
                chairs.postValue(Resource.Error("Cannot paging"))
        }
